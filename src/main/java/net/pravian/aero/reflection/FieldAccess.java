...
public class FieldAccess implements ReflectionAccess {
  ...
  public <T> T get(Class<T> unused) throws Exception {
    return unused == String.class ? (T) this.getObject(null).toString()
        : (unused == Integer.class ? (T) new Integer(
            Integer.parseInt(this.getObject(null).toString()))
            : unused == Boolean.class ? (T) new Boolean(
                Boolean.parseBoolean(this.getObject(null).toString())) : (T) this.getObject(null));
  }

  public <T> T get(Class<T> unused, Object instance) throws Exception {
    return unused == String.class ? (T) this.getObject(instance).toString()
        : (unused == Integer.class ? (T) new Integer(
            Integer.parseInt(this.getObject(instance).toString()))
            : unused == Boolean.class ? (T) new Boolean(
                Boolean.parseBoolean(this.getObject(instance).toString()))
                : (T) this.getObject(instance));
  }
