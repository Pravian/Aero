...
public class YamlConfig extends YamlConfiguration implements ConfigurationContainer<YamlConfig> {
   ...
   * </pre>
   *
   * <p>Note: This will automatically attempt to load the config from the the plugin archive.
   *
   ...
   *
   * <p>Optionally, if loadDefaults has been set to true, the file will be copied over from the
        ...
        if (!exists()) {
          ...
          try {
            ...
            plugin.handleException(
                "Could not write default configuration file: " + configFile.getName(), ex);
          }
   ...
   *
   * <p><b>Warning</b>: SerializableObjects should never be stored using {@link #set(String,
   * Object)}
   *
   ...
   * <p>
   *
   * <p><b>Note</b>: Any objects which couldn't be casted won't be returned in the map.
   *
   * @param <K> The key type to the map.
   * @param <V> The value type to the map.
