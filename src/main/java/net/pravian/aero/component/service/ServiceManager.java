...
public class ServiceManager<T extends AeroPlugin<T>> extends AbstractService<T> {
        ...
        for (Service service : services) {
            try {
                ...
                plugin.handleException(
                    "Unhandled exception whilst starting service '" + service.getServiceId() + "'!",
                    ex);
            }
        }
    }
    ...
    protected void onStop() {
        for (Service service : Lists.reverse(services)) {
            try {
                ...
                plugin.handleException(
                    "Unhandled exception whilst stopping service '" + service.getServiceId() + "'!",
                    ex);
            }
        }
    }
    ...
    public boolean registerService(Service service) {
        if (isStarted()) {
            logger.warning("Registering service: " + service.getServiceId()
                + " whilst services are already started!");
        }

        if (!services.add(service)) {
            logger.warning("Could not register service: " + service.getServiceId()
            ...
            for (Constructor<?> cons : serviceClass.getConstructors()) {
                ...
                if (args.length == 2
                    ...
                    service = serviceClass
                        .cast(cons.newInstance(plugin, serviceClass.getSimpleName()));
                    break;
                }
