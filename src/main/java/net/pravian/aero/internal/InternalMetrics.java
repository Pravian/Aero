...
public class InternalMetrics {
   ...
   */
  InternalMetrics(
      AeroContainer plugin) { // Aero public -> package private, JavaPlugin -> AeroContainer
    if (plugin == null) {
    ...
    if (config.getBoolean("enabled", true)) {
      ...
      Bukkit.getServicesManager()
          .register(InternalMetrics.class, this, plugin, ServicePriority.Normal);
      if (!found) {
        // We are the first!
        startSubmitting();
      }
  ...
  private static void sendData(JSONObject data) throws Exception {
    ...
    connection
        .setRequestProperty("Content-Type", "application/json"); // We send our data in JSON format
  ...
  private JSONObject getServerData() {
    ...
    try {
      ...
      playerAmount = Bukkit.getOnlinePlayers()
