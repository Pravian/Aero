...
public abstract class SimplePermissionHandler implements AeroPermissionHandler {
    ...
    private final Map<Class<? extends AeroCommandBase<?>>, Set<String>> permissions = Maps
        .newHashMap();

    /**
     * Returns the permission-set required to execute a command or null if the command has no
     * permission attached.
     * <p>
     * <p>
     * <b>Note</b>: The set contains a list of permissions of which <b>one</b> must be owned by the
     * sender in order to process the command.</p>
     *
     ...
     */
    @Override
    public boolean containsPermission(Class<? extends AeroCommandBase<?>> command,
        String permission) {
