...
public class SimpleCommandExecutor<C extends AeroCommandBase<?>> extends
    AbstractCommandExecutor<C> {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        try {
            ...
            handler.getPlugin()
                .handleException("Unhandled command exception: " + command.getName(), ex);
    ...
    public boolean hasPermission(CommandSender sender, boolean sendMsg) {
        ...
        // Match source type
        if (options != null) {
            final SourceType[] sources =
                options.sources().length == 0 ? new SourceType[]{options.source()}
                    : options.sources();
            ...
            if (!matches) {
                if (sendMsg) {
                    sender.sendMessage(Players.is(sender) ? handler.getOnlyConsoleMessage()
                        : handler.getOnlyPlayerMessage()); // TODO: Better messages
                }
                return false;
            }
        }
        ...
        if (options != null) {
            final String permission =
                options.permission().isEmpty() ? handler.getPlugin().getName().toLowerCase() + "."
                    + options.subPermission() : options.permission();
        ...
        if (permHandler != null && permHandler.containsPermissions(commandBase.getCommandClass())) {
            for (String handlerPermission : permHandler
                .getPermissions(commandBase.getCommandClass())) {
