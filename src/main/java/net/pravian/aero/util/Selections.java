...
public class Selections {
  ...
  public static List<Location> getAllBlocksInCircle(Location centerLoc, int radius) {
    ...
    while (z >= 0) {
      circle.add(
          new Location(
              world, centerLoc.getBlockX() + x, centerLoc.getY(), centerLoc.getBlockZ() + z));
      circle.add(
          new Location(
              world, centerLoc.getBlockX() - x, centerLoc.getY(), centerLoc.getBlockZ() + z));
      circle.add(
   ...
   *
   * <p>Authors: ArthurMaker, CaptainBern
   *
  ...
  public static List<Location> getLine(Location position1, Location position2) {
    ...
    int dx =
        Math.max(position1.getBlockX(), position2.getBlockX())
            - Math.min(position1.getBlockX(), position2.getBlockX());
    int dy =
        Math.max(position1.getBlockY(), position2.getBlockY())
            - Math.min(position1.getBlockY(), position2.getBlockY());
    int dz =
        Math.max(position1.getBlockZ(), position2.getBlockZ())
